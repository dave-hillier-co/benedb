...
import { buildRandomAuthzWorld } from "@spacedb/engine/random-authz-worlds";
 ...
 * container) and to SpaceDB (in-process, through the real grain mesh - `MeshTestCluster`), then
 ...
 * compares SpaceDB's engine against SpaceDB's OWN engine over a different code path (the reference
 ...
 * semantics baked into `@spacedb/engine` - a bug in THAT shared understanding would pass every one
 * of them. This suite is the only one that can catch that class of bug, because the oracle (a real
 * `authzed/spicedb` binary) shares no code with SpaceDB.
 *
 ...
 *     into a non-member. This reset is what makes the suite deterministic. The SpaceDB side needs no
 ...
 *     `WriteRelationships` in batches of 50 over the real wire; SpaceDB gets a direct
 *     `datastore.readWriteTx(tx => tx.writeRelationships(updates))`. Routing the SpaceDB side
 *     through gRPC would change what is being compared.
 *  6. `FakeServerCallContext` DISAPPEARS: SpaceDB's service methods take a trailing OPTIONAL
 ...
 *
 * WHEN THIS SUITE DISAGREES: do not reflexively "fix" SpaceDB. A divergence here may be a SPICEPORT
 * defect that SpaceDB faithfully transliterated - check the open spiceport issues and record the
...
async function writeRelationships(
  ...
  // SpaceDB side: straight to the datastore transaction (the same path WriteRelationships's gRPC
  ...
  it.for(SEEDS_LOCAL)(
    ...
    async (seed, ctx) => {
      ...
      try {
        ...
        // SpaceDB's cluster is already compiled with this schema at startup, so there is nothing
        ...
        for (const permission of PERMISSIONS) {
          for (const [resourceId, subjectId] of enumerateCheckQueries(
            ...
            const spacedbResp = await permissionsService.checkPermission(
              CheckPermissionRequest.fromPartial({
                resource,
                permission,
                subject,
            ...
            if (
              ...
              normalizePermissionship(spacedbResp.permissionship)
            ) {
              failures.push(
                `seed=${seed} CheckPermission document:${resourceId}#${permission}@user:${subjectId} ` +
                  `spicedb=${spiceDbResp.permissionship} spacedb=${spacedbResp.permissionship}`,
              );
            }
          }
        }
        ...
        expect(
          failures.length === 0,
          `Divergence(s) between real SpiceDB and SpaceDB:\n${failures.join("\n")}`,
        ).toBe(true);
      } finally {
        spiceDbClient.close();
